...
/// 黄道 of date（IAU2006 平均黄道・平均分点）→ GCRS の回転行列。
/// SOFA/ERFA `iauEcm06`（ICRS→黄道 of date）の**転置**。TT 引数。
/// `v_gcrs = M · v_ecliptic_of_date`、`M = Pmat06(t)ᵀ · R1(−ε_A)`
/// （`iauEcm06 = R1(ε_A)·Pmat06`、その転置）。
/// 黄道座標の暦（VSOP87D 太陽 = 黄道 of date、ELP 月 = 黄道 J2000 は J2000 で本行列を使う）を
/// GCRS へ載せる（ISSUE-015 見かけ位置の前段。光行時間・光行差・章動は後続）。
pub fn ecliptic_to_gcrs_matrix(time_tt: TtInstant) -> Matrix3 {
    let epsa = mean_obliquity_iau2006(time_tt).0;
    // M = Pmat06ᵀ · R1(−ε_A) = (R1(ε_A)·Pmat06)ᵀ = iauEcm06ᵀ。
    bias_precession_matrix_iau2006(time_tt)
        .transpose()
        .mul_mat(&Matrix3::rotation_x(-epsa))
}
...
mod tests {
    ...
    // ==================================================================
    // 1d. 黄道 of date → GCRS 行列（ISSUE-015 S1, ecm06ᵀ）
    // ==================================================================
    //
    // オラクル: pyerfa `erfa.ecm06(jd, 0.0)` は GCRS→黄道of date の回転行列。
    // 本実装 ecliptic_to_gcrs_matrix は逆（黄道of date→GCRS）なので ecm06 の**転置**。
    // 各定数は erfa.ecm06(jd,0.0).T の 9 要素を行優先で逐語転記（pyerfa = 独立実装）。
    // 許容 1e-12: 本行列は既検証の pmat06(1e-14)・obl06(1e-14) の単純合成で章動の表現差を
    // 含まないため erfa.ecm06ᵀ と f64 演算誤差で一致。実バグは要素を ≫1e-6 ずらす。
    const ECM_TOL: f64 = 1e-12;

    #[test]
    fn ecliptic_to_gcrs_matches_erfa_ecm06_transpose() {
        const J2000: [[f64; 3]; 3] = [
            [
                0.9999999999999941,
                3.2897004077419646e-08,
                -1.0207044725484355e-07,
            ],
            [
        ...
        const JD_1900: [[f64; 3]; 3] = [
            [
                0.9997029465818913,
                -0.024372488886453318,
                -1.951829760774189e-05,
            ],
            [0.0223513541760769, 0.9171194100108935, -0.39798543283362275],
            [
                0.009717796148294214,
                0.39786677364005935,
                0.9173917346865896,
            ],
        ];
        const JD_2050: [[f64; 3]; 3] = [
            [
                0.9999256883794357,
                0.012190882354620196,
                1.0311515302950311e-05,
            ],
            [
                -0.011181366274202987,
                0.917459102145863,
                -0.39767294720404217,
            ],
            [
                -0.004857444508551175,
                0.39764328018605155,
                0.9175272349939939,
            ],
        ];
        const JD_1600: [[f64; 3]; 3] = [
            [
                0.9952570786129615,
                -0.09727971556127042,
                -6.641620175201063e-05,
            ],
            [
                0.08919074138812863,
                0.9127719029003099,
            ...
            [0.03883759932629072, 0.3967166623877368, 0.9171191474734921],
        ];
        const JD_2100: [[f64; 3]; 3] = [
            [
                0.999702691906827,
                0.024382931428034174,
                2.1194797574869417e-05,
            ],
            [
                -0.022364678091227327,
                0.9172996836728771,
                -0.3975690021964935,
            ],
            [
                -0.009713339699580005,
                0.3974503276997208,
                0.9175722794653646,
            ],
        ];
        for (label, jd, expected) in [
            ("J2000", 2451545.0, &J2000),
            ("1900", 2415020.0, &JD_1900),
            ("2050", 2469807.0, &JD_2050),
            ("1600", 2305447.5, &JD_1600),
            ("2100", 2488069.5, &JD_2100),
        ] {
            let m = ecliptic_to_gcrs_matrix(tt(jd, 0.0));
            assert!(
                matrix_close(&m, expected, ECM_TOL),
                "ecm06ᵀ mismatch at {label} (jd={jd}); got {:?}",
                m.rows
            );
        }
    }

    /// ecliptic_to_gcrs_matrix は純回転 → 直交（M·Mᵀ ≈ I）。
    #[test]
    fn ecliptic_to_gcrs_is_orthonormal() {
        assert_orthonormal(&ecliptic_to_gcrs_matrix(tt(2451545.0, 0.0)), 1e-12);
    }
}
