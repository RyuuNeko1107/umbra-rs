...
//!   （greatest のみ）。
//! - 北/南限界線・部分食域・samples は features に出さない（本スライス）。
...
/// center_line=Some の中心食 EclipsePath を作る（限界線・部分食域 None・samples 空）。
...
fn central_path() -> EclipsePath {
    EclipsePath {
        ...
        northern_limit: Some(GeoLine::new(vec![pt(1.0, 1.0)])),
        southern_limit: Some(GeoLine::new(vec![pt(2.0, 2.0)])),
        partial_limit: None,
...
// ============================================================
// to_geojson — 中心食（center_line=Some）: greatest と center_line 両方
// ============================================================

/// 中心食（center_line=Some）の features に role=greatest の Point と
/// role=center_line の Feature が**両方ちょうど 1 つずつ**ある。
/// 北/南限界線・部分食域・samples は features に出ない（本スライス）。
///
/// 殺す変異: center_line feature を出さない・greatest を出さない・限界線/samples を features に
///   混ぜる・role 文字列の取り違え（greatest↔center_line）。
#[test]
fn to_geojson_central_has_greatest_and_center_line_features() {
    let s = central_path().to_geojson().expect("直列化は成功する");
    ...
    assert_eq!(
        greatest.len(),
        1,
        "role=greatest の feature はちょうど 1 つ"
    );
    let center = features_with_role(&root, "center_line");
    assert_eq!(
        center.len(),
        1,
        "role=center_line の feature はちょうど 1 つ"
    );

    // greatest feature は Point geometry・かつ Feature 型。
    assert_eq!(
        greatest[0]["type"],
    ...
    // center_line feature は LineString geometry（跨ぎ無し中心線）。
    assert_eq!(
        center[0]["geometry"]["type"],
        Value::String("LineString".to_string()),
        "center_line の geometry は LineString（跨ぎ無し）"
    );

    // 本スライスでは features は greatest + center_line のちょうど 2 件
    //（限界線・部分食域・samples は出さない）。
    let all = root["features"].as_array().expect("features は配列");
    assert_eq!(
        all.len(),
        2,
        "features は greatest + center_line の 2 件のみ（限界線/samples は出さない）, got {}",
        all.len()
    );
}
...
/// 中心線が日付変更線を跨ぐ（lon=[170, -170, -160]）と center_line feature の geometry は
/// MultiLineString になる（umbra-geo の GeoLine 分割が EclipsePath 経由でも効く）。
///
