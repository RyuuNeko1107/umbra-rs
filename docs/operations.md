...
2. **探索性能は内部粗スキャン（非公開・候補棄却専用）＋並列で追求**（局地多項式化に依存しない）。
   - 100 年 ≈ 1240 朔。各朔は内部粗スキャン暦（簡易・非公開）で合 solver＋早期棄却（偽陰性ゼロのマージン: physical/accuracy）。**棄却に残った候補の日食/接触は必ずフル Standard で再計算**（~240 件のみフル Standard ベッセル）。100 年検索が数秒に収まる設計。
3. **項数階層**: `AnalyticalEphemeris` は級数打切りを切替（内部粗スキャン=少項（候補棄却専用・非公開）/ Standard=フル / Reference=DE）。公開プロファイルは Standard/Reference の2層で、少項は内部粗スキャン専用。
