...
**(8.11)** 食帯幅（km, 中心食のみ Some）＝**南北本影縁の地表点間 大圏距離**（M9.6 実装）:
```
w_path = great_circle( P_north , P_south )    [km]
P_north/south = solve_limit_edge(t_max, sign=±1)   （M9.4 相対速度包絡の南北限界点）
```
- 帯幅は南北限界線（§限界線・M9.4）の定義量そのもの＝両縁の地表点間距離。基本面半径 |L2'| の素朴投影でなく、**錐∩楕円体の厳密縁点**を地表で測るため太陽高度傾斜・扁平を厳密に含む。出典: 限界線（ExplSup §11 / NASA path width）。数値オラクル NASA 2024 = 197.5 km（実装 [185,215] km で検証）。**「要確認」解消**（限界線の定義量・式番号転記不要）。

**(8.12)** 中心食継続時間（秒, 中心食のみ Some）＝**本影直径 ÷ 影の地表相対速度**（M9.6 実装・初等運動学）:
```
D_cen = 2·|L2'| / |rel|   [hour]  → ×3600 [s]
|L2'| = |l2 − ζ·tan f2|（中心軸 ζ）
rel   = ( x' − μ'(ζ cos d − η sin d) , y' − μ'·ξ sin d )   （影軸運動 − 観測者自転運搬 ω×P・中心軸 ξ=x,η=y）
```
- 中心線上の点を本影（基本面で半径 |L2'| の円）が通過する時間＝直径 2|L2'| ÷ 影の地表相対速さ |rel|。`rel` は M9.4 包絡と同式（地球自転 μ' 込み）。x'/y'/μ' は供給源の数値中心差分（直接評価源のため）。出典: 初等運動学（直径÷速度）・NASA duration で検証（2024 = 268.1 s、実装 [250,286] s）。**「要確認」解消**（第一原理導出・式番号推測転記なし）。
- **部分食・非中心では w_path = None, D_cen = None**（ISSUE-023, Option 設計）。`|rel|` 非正/非有限は None（縮退）。
