...
- **残: (3c-ii) 外周組立**: (3c-i) 半影限界＋(3b) rise/set 曲線を 11.4 の順序で連結し `GeoPolygon` 外環を構成。
  反子午線 MultiPolygon 分割。`partial_limit` を `path()` で Some に（部分食 only の日食含む）。オラクル: 実 2024 で
  外環が NASA 部分食限界の ballpark を内包・partial ⊃ umbral path、頂点数・閉性・環向き。要確認 #3（単連結性）/#4（NASA 粒度）。
