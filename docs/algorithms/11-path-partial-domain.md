...
- **(3f) full containment＝3 領域の多角形ユニオン** ✅（2026-09-14・strict・§11.6/§11.7）: 部分食域を
  ...
  **中心線全点内包を headline acceptance に昇格**＝実 2024-04-08 で 194 点全て内包（従来 190/194）。
  リボン位相の契約は撤廃（§11.6(e)）。mutation: union 275 変異 234 caught・35 missed（31 は等価・
  3 系統は未解決＝要確認 7）・`docs/reviews/mutation-polygon-union.md`。
...
7. **(3f) 多角形ユニオンに、受け入れオラクルが到達していないロジックが残る（2026-09-14・要判断）**。
   `cargo mutants`（275 変異・234 caught・5 timeout・**35 missed**）のうち 31 件は等価変異として許容できるが、
   次の 3 系統は**等価ではないのに撃てていない**（`docs/reviews/mutation-polygon-union.md` B 節）:
   - **分岐頂点の後継半辺選択**（`clip.rs` の `back - atan2(…)`）— 角度の符号を反転しても、非対称な点接触・
     3 領域会合の合成テストでも実 2024 SLOW でも差が出ない。
   - **穴の割当**（「穴を含む**最小面積**の外環へ」の比較）— 差が出るのは穴が 2 つ以上の外環に含まれる
     **二重入れ子**（穴の中の島がさらに穴を持つ）配置のみで、現行テストも実データもその構造を作らない。
   - **共線部分重なりの分割点登録**（`param_on_segment`）— 必要な分割点が非平行交差またはリング頂点として
     別経路で供給されるため、現行の全配置で差が出ない。

   **判断が要る点**: 上記を (a) 判別テストを構成して撃つ、(b) 到達不能と確定したうえで該当ロジックを削るか
   不変条件で防御する、のどちらに倒すか。`union_rings` は `pub` API なので「実装はあるが証明されていない」
   状態を残したままにしない。部分食域の許容誤差（ballpark 数十 km）と「最大面積成分のみ使う」運用のため
   実害は限定的だが、契約としては未証明である。

8. **(3f) ユニオンが複数成分に割れたときの 2 番目以降の切り捨て（2026-09-14・要判断）**。
