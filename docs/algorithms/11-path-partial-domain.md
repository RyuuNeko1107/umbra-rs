...
### 11.4 外周ポリゴンの構成 — 式 11.4（**最大食点まわりの方位ソート**・要確認#3 の方針確定）

外環を成す境界点は 2 種: **昼面の南北半影限界**（11.1・(3c-i) `trace_penumbral_limits`）と **terminator 上の
rise/set limb 点**（11.3・(3b) `cone_terminator_intersections`）。これらを [P1,P4] にわたり収集し、**最大食点
`greatest_point` を極とする大圏方位（bearing）で昇順ソート**して外環の頂点列とする。

**なぜ方位ソートか（4 曲線ステッチに対する選択）**: 「北半影限界→夕 limb→南半影限界→朝 limb」を端点で繋ぐ
4 曲線ステッチは、半影限界が terminator に達して途切れる端（`RootNotBracketed`）と limb 曲線端の接合が
geometry 依存（半影が昼面に収まる/terminator を跨ぐ/極に達する等で位相が変わる）で**脆い**。一方、部分食域は
半影円の時間掃引の和集合で `greatest_point` 近傍を常に含む**おおむね star-shaped な単一塊**なので、極からの方位
ソートは連結ギャップに依らず頑健に閉環を与える（resolution 機構の発想に近い）。

**方位ソート手順**:
1. 全境界点 P_i（南北半影限界＋rise/set limb 点）を収集。
2. 各 P_i の `greatest_point` からの初期大圏方位 `az_i = bearing(greatest_point → P_i)`（経度ラップに非依存）を計算。
3. `az_i` 昇順にソート＝外環頂点列（CCW/CW は az の向きで決まる・RFC 7946 へは (3d) で正規化）。

**前提・限界（要確認#3 の方針）**: `greatest_point` から見て領域が star-shaped（各方位に境界点が一意）。通常の
中緯度日食は単一塊で成立。**非 star-shaped になりうる退化**（grazing で領域が極側の薄片・領域が半球超で
方位が一意でない）は v1 の対象外＝**近似明記**（conventions §11）。これらは穴・MultiPolygon を含む後続厳密化で扱う。

- 反子午線（|Δlon|>180）跨ぎ MultiPolygon 分割・環向き RFC 7946（外環 CCW・穴 CW）正規化は **(3d)** GeoJSON 化で扱う
  （`partial_limit` 自体は方位ソートの単一外環として持つ）。

### 11.5 検証戦略（弱オラクルの補強）

外環の topology（自己交差なし・正しい巻き）は方位ソートの star-shaped 前提に委ねるため、頂点の正しさと領域の
妥当性を多層で縛る:
- **頂点の正当性**: 各外環頂点は半影縁条件（軸からの面内距離 ≈ `|l1−ζ·tan f1|`）**または** terminator 条件（ζ≈0）の
  いずれかを機械精度で満たす（11.1/11.3 のオラクルを頂点単位で）。
- **包含**: 中心食では `partial_limit` が中心線・最大食点を内包（球面 point-in-polygon）＝ partial ⊃ umbral path。
- **非退化**: 外環は ≥3 頂点・閉・正の面積。
- **実 2024 ballpark**: 領域の南北端緯度が NASA 部分食北限/南限の桁に整合・既知の部分食目撃都市を内包し遠方点を除外。
...
- **残: (3c-ii) 外周組立**: (3c-i) 半影限界＋(3b) rise/set limb 点を収集し、**最大食点まわりの大圏方位ソート**
  （11.4・star-shaped 前提）で `GeoPolygon` 外環を構成。`partial_limit` を `path()` で Some に（部分食 only の日食含む）。
  検証は 11.5（頂点が半影縁 or terminator 条件・partial ⊃ umbral path・非退化・実 2024 ballpark）。反子午線
  MultiPolygon・環向き正規化は (3d)。非 star-shaped 退化は近似明記（要確認#3 の方針）。
...
3. ~~部分食域の単連結性~~ **方針確定（11.4・v1）**: 最大食点まわりの**方位ソート**で単一外環を構成＝
   star-shaped 前提（通常の中緯度日食で成立）。非 star-shaped 退化（grazing の極側薄片・半球超）は v1 対象外＝
   近似明記。穴・MultiPolygon を含む厳密化は後続。
