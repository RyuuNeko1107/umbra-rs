...
## 6. OSS公開前に解決する未確認事項（チェックリスト）

- [ ] VSOP87 原データの再配布可否（IMCCE/CDS の利用条件を一次確認）
- [ ] ELP/MPP02 原データの再配布可否（IMCCE の利用条件を一次確認）。GPL再実装の非混入を保証
- [ ] 本crate のライセンス確定（MIT / Apache-2.0 / デュアル）と、係数データのライセンス整合
- [ ] JPL DE を同梱しない方針の確認（任意DL前提で機能が成立すること）
- [ ] IERS EOP / 閏秒の同梱条件・更新運用（valid_to 期限切れ時の挙動）
- [ ] NASA / USNO オラクル転記の範囲（数値事実のみ・出典明記）の妥当性
- [ ] `cargo-deny`（ライセンス allow-list）/ `cargo-audit` を CI に組込み、依存とデータのライセンスを機械チェック
