...
- モード(tdd-workflow): **strict**（公開仕様: フレーム連鎖の定義・Standard 必須ON・採用モデル（2006/2000A vs Fast 2000B）。conventions §5 / accuracy.md §1。strict）

## 目的
地球姿勢に基づくフレーム変換連鎖を実装する: `GCRS →[frame bias + 歳差(IAU2006) + 章動(IAU2000A)]→ CIRS →[ERA(UT1)]→ TIRS →[極運動]→ ITRS`（conventions §5）。見かけ位置（ISSUE-015）とベッセル要素生成（umbra-eclipse）が利用する。Standard では IAU2006/2000A を必須、Fast のみ IAU2000B を許容（architecture §5）。
...
- Fast 用: `pub fn gcrs_to_cirs_matrix_2000b(time_tt) -> Matrix3`（IAU2000B 章動）。
...
- **IAU2000A 章動**: Mathews, Herring, Buffett (2002) / IAU2000A。SOFA: `iauNut00a`（Δψ, Δε, 1365項級数）。Fast 用 IAU2000B: `iauNut00b`（77項）。
...
- 座標系: GCRS / CIRS / TIRS / ITRS（conventions §5 表）。CIO ベース変換で統一（赤道分点ベースの恒星時は使わない。一貫性のため ※要確認: 恒星時版を Fast に許すか）。

## アルゴリズム概要
1. GCRS→CIRS: `iauC2i06a` 相当（X,Y,s から CIP/CIO 行列）。Fast は 2000B 章動で X,Y を近似。
...
- IAU2000A vs 2000B 差分が既知オーダー（~1mas 級）に収まることを確認（Fast 許容の根拠）。
...
- Fast の IAU2000B は ~1mas 級の追加誤差だが Fast プロファイル目標（最大食概算 数分以内）に対し無視可。
...
- Standard は IAU2006/2000A を EngineConfig で固定。Fast のみ 2000B 切替を許可し、選択を `CalculationMetadata`（ephemeris/フレームモデル欄）へ記録。
