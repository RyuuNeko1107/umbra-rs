...
- **残（後続スライス）**: (2b) `samples`（PathSample 列＝経路各点の time/center/duration/sun_altitude/path_width/kind。`EclipsePath.samples` は現状空）、(3) 部分食域（部分食限界 `GeoPolygon`＝半影包絡＋日の出入り食限界曲線・**要一次資料**）＋その GeoJSON feature 化（`GeoPolygon::geojson_geometry`・partial_limit を to_geojson に・ポリゴンの ±180 は MultiPolygon 化）。
