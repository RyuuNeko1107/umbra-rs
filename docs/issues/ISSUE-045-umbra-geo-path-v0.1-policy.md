...
- **残（後続スライス）**: (3) 部分食域（部分食限界 `GeoPolygon`＝半影包絡＋日の出入り食限界曲線）＋その GeoJSON feature 化（`GeoPolygon::geojson_geometry`・partial_limit を to_geojson に・ポリゴンの ±180 は MultiPolygon 化）。**設計ドラフト完了**（2026-06-21・`docs/algorithms/11-path-partial-domain.md`）＝第一原理で境界を導出（南北半影限界＝M9.4 本影限界の l1 一般化／rise-set 曲線＝半影縁∩terminator 二円交点／外環組立＋反子午線 MultiPolygon）。サブスライス (3a) 半影限界の半径引数化・(3b) rise/set 二円交点・(3c) 外環組立＋partial_limit 充足・(3d) GeoPolygon GeoJSON に分解。要確認 5 件（rise/set 4 分類の取捨・terminator の WGS84 厳密化・単連結性・NASA limits 表入手・solve_limit_edge 一般化の M9.4 mutation 影響）。NASA 2024-04-08 をオラクルに第一原理導出（M9.4/M9.6 同方針・一次資料の式番号は未確認のため未転記）。
