...
- **後続（要対応・独立スライス）**: text 整形 `format_search_text` は秒を `{:04.1}` で素通しするため、
  `jd2_to_gregorian` の境界 ±eps（例 15:59:59.9995）で `15:59:60.0` の不正表記を出しうる。json 経路は
  `umbra_core::time::iso_body_tenths`（0.1 秒丸め＋分/時/日繰り上げ）で解消済みだが、text 経路は未対応。
  text 側にも同等の丸め・繰り上げを適用する（テスト先行・別スライス）。
