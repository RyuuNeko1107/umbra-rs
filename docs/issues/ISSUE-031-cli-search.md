...
## スライス進捗 / 後続
- **S31a（済）**: `umbra search` text 出力。
- **S31b（済）**: `--format json`。serde 横断配線（`SolarEclipse` 推移閉包に Serialize を feature ゲートで付与・
  列挙 `{type:..}`／単位フィールド名・A7／時刻 `{iso, jd}`／GeoPoint `{lat_deg, lon_deg}`）＋ `serde_json` 配列出力。
- **text 0.1 秒丸め（済・独立スライス）**: text 整形 `format_search_text`/`format_local_text` が
  秒を `{:04.1}` で素通しし、`jd2_to_gregorian` の境界 ±eps（例 15:59:59.9995）で `15:59:60.0` の
  不正表記を出していた問題を解消。共有公開ヘルパ `umbra_core::jd2_to_gregorian_deciseconds`
  （0.1 秒丸め＋分/時/日/月/年 繰り上げ）を新設し、両 text 整形と `iso_body_tenths`（json）を本関数へ
  統一。表示専用で lossless 値（`jd` チャネル）・内部計算は full-precision のまま。
