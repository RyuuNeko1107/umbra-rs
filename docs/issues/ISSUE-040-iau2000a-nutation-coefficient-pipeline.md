...
- 併せて Fast 用 IAU2000B（`nut00b`, 77 項）も同パイプラインで管理可能にする（既定は 2000A。要確認: 2000B を同 target に含めるか別 target か）。
...
- 打切り: IAU2000A は**全項採用**（accuracy.md §2.1 フレーム 0.05″ 配分に対し実力 ~1mas で余裕、打切りは Fast の 2000B 側で対応）。2000A は項を間引かない（精度最優先方針）。
...
5. （任意）`nut00b` 77 項も同手順（Fast 用、ISSUE-035 `iauNut00b` 相当）。
