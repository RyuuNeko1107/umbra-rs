...
- 偽陽性は可（後段で棄却）、**偽陰性は不可**（朔を1件も落とさない。探索は偽陰性ゼロ＝accuracy.md §3.4 / architecture §3）。
...
## 実装メモ
- 窓半幅 Δ は `EngineConfig` 由来にはせず本層の定数（根拠コメント付き）。将来 内部粗スキャン/Reference で刻みを変えるなら設定化（要レビュー）。
- 概算に使う暦は精度不問のため、内部粗スキャン相当の簡易評価で良い（Standard 暦をフルに回すのは無駄）。ただしバックエンド抽象（`impl Ephemeris`）は維持し、MockEphemeris でもテスト可能にする。
